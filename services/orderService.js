...
const placeOrderService = async (user_id, cart_id) => {
  const cartItems = await getCartItemsFromDb(user_id, cart_id);
  ...
  const orderResult = await insertOrderInDb(user_id, cart_id, totalAmount);
  const orderId = orderResult.insertId;

  await insertOrderItemsInDb(orderId, cartItems);
  await clearCartInDb(user_id, cart_id);

  return { orderId, totalAmount, message: "Order placed successfully!" };
};

const getUserOrdersService = async (user_id) => {
  return await getUserOrdersFromDb(user_id);
};
