...
router.post('/', (req, res) => {
    ...
    connection.query(query, [name, icon, color], (err, results) => {
        ...
        res.status(201).json({ success: true, category: { id: results.insertId, name, icon, color } });
    });
});
...
router.put('/:id', (req, res) => {
    const { name, icon, color } = req.body;
    const query = 'UPDATE categories SET name = ?, icon = ?, color = ? WHERE id = ?';
    
    connection.query(query, [name, icon, color, req.params.id], (err, results) => {
        if (err) {
